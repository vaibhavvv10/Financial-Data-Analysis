...
def</span> calculate_rsi(prices, window=14):

    delta = prices.diff()

    gain = (delta.where(delta > 0, 0)).rolling(window=<span class="cm-variable">window).mean</span>()
...
fig.add_trace(go.Scatter(
    ...
    title=</span>"RSI for RELIANCE.NS",
...
fig = go.Figure(data=[go.Table(
    ...
    cells=dict(values=[table_data</span>['Stock'], table_data['Sharpe Ratio']],
...
<span role="presentation" style="padding-right: 0.1px;">fig.update_layout(
...
last_price = nifty50_data['RELIANCE.NS'].iloc[-1</span>]
...
for i in range(num_simulations):
    ...
    fig.add_trace(go.Scatter(
        ...
        y<span class="cm-operator">=simulated_prices[i],

        mode='lines',

        line=dict</span>(width=0.5),
    ...
    template<span class="cm-operator">="plotly_white"